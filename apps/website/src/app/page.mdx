...
If you're using Tailwind, import `@tenstorrent/vesper/tailwind.css` into your Tailwind config:
...
1. `yarn compile:tsc` compiles source files to ESM plus declarations
2. `yarn compile:alias` rewrites emitted import specifiers to relative ESM-compatible paths with `.js` extensions
3. `yarn compile:css` copies every side-effectful file file under `src/` into the matching location under `dist/`
