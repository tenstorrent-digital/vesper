...
A polymorphic text component that applies consistent typographic styles from the Vesper design system. Supports a wide range of display, heading, copy, and label variants. [View in Storybook]({`${storybookUrl}/typography`})

<div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
  <Typography variant="display-lg">Display Large</Typography>
  <Typography variant="heading-md">Heading Medium</Typography>
  <Typography variant="copy-md">Body copy for paragraphs and general content.</Typography>
...
```tsx
// Monospace text
<Typography variant="copy-xs-mono">
  const x = 42;
</Typography>
```
