...
A minimal, text-only button without background or border. Supports multiple color variants, sizes, and optional leading or trailing icons. Ideal for inline actions and links. [View in Storybook]({`${storybookUrl}/text-button`})
...
```tsx
// With icons
import { ArrowRight, Download } from "@repo/vesper/icons";
