...
Color tokens are split into **primitive** tokens (raw palette values) and **semantic** tokens (aliases of primitive tokens, intended for day-to-day usage). For example, the semantic token `--vesper-background-primary` token aliases the primitive `--vesper-stone-900` color token. Prefer semantic tokens wherever possible, since they automatically resolve to the correct primitive for the active theme.
