 ...
 *
 * Only the fields shiki reads are kept. `normalizeTheme` in
 * `@shikijs/primitive` consults `name`, `type`, `tokenColors` (as `settings`),
 * `colors["editor.foreground"]` and `colors["editor.background"]`, while
 * `colors["terminal.ansi*"]` is read by `tokenizeAnsiWithTheme` for
 * `lang="ansi"`. Every other workbench color from the upstream theme, plus
 * `displayName` and `semanticHighlighting`, is dropped because shiki never
 * looks at it. Within `tokenColors`, only the `foreground`, `background`, and
 * `fontStyle` settings are meaningful.
 *
