...
export const createMask = (
  ...
  if (element.tagName !== "INPUT" || !ALLOWED_TYPES.includes(element.type)) {
    return () => {};
  }

  const { initialValue = "" } =
    (element as { _wrapperState?: { initialValue?: string } })._wrapperState ??
    {};
  ...
  const onInput = (event: Event) => {
    try {
      ...
      element.setSelectionRange(tracking.selectionStart, tracking.selectionEnd);
