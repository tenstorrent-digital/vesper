...
- **Choose an auth path:**
  - **Trusted publishing / OIDC (recommended).** No long-lived secret, and provenance is attached automatically. Configure it on npmjs.com under the package's Settings → Trusted Publisher: GitHub Actions, repo `tenstorrent-digital/vesper`, workflow `release.yml`. Requires npm >= 11.5.1; Node 24 ships npm 11.8.0, so CI already satisfies this. Note that a trusted publisher is configured against an _existing_ package, so the first `0.1.0` publish likely has to happen manually or with a short-lived token.
