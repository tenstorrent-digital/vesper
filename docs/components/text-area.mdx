...
Form props (`value`, `disabled`, `name`, `form`, etc.), input props (`autoCorrect`, `spellCheck`, `autoFocus`, etc.), `aria-*` attributes, `ref`, and event handlers related to `focus`, `blur`, `scroll`, `input`, and `change` events are forwarded to the underlying `<textarea>` element. All other props are forwarded to the wrapping `<div>` element.
...
<TextArea aria-label="About" placeholder="Enter some text" />;
```
