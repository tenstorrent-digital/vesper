...
Buttons can be rendered in several different variants:
...
Buttons can be rendered in one of four sizes; `"xs"`, `"sm"`, `"md"`, or `"lg"`. The default size is `"md"`.
...
Buttons can render icons to the left or the right of their content via the `iconLeft` and `iconRight` props. Simply pass your icon as a `ReactNode` to either of these props:
