...
| ------------ | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| `"primary"`  | <IconButton variant="primary" icon={<Tenstorrent />} />  | Default variant. Use for the main action a user is expected to take in a given context.                       |
| `"subtle"`   | <IconButton variant="subtle" icon={<Tenstorrent />} />   | Use for secondary actions that are important but should have less visual prominence than the primary action.  |
| `"tertiary"` | <IconButton variant="tertiary" icon={<Tenstorrent />} /> | Use for secondary or supporting actions that need a clear boundary from surrounding content.                  |
| `"contrast"` | <IconButton variant="contrast" icon={<Tenstorrent />} /> | Use when an action needs strong visual contrast against its surrounding content.                              |
| `"ghost"`    | <IconButton variant="ghost" icon={<Tenstorrent />} />    | Use for low-emphasis actions where a button's presence should be unobtrusive.                                 |
| `"danger"`   | <IconButton variant="danger" icon={<Tenstorrent />} />   | Use for destructive or potentially irreversible actions that could result in negative consequence.            |
| `"warning"`  | <IconButton variant="warning" icon={<Tenstorrent />} />  | Use for actions that may have meaningful consequences or require caution, but are not inherently destructive. |
