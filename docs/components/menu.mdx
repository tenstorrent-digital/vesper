...
Use a `Menu` for a list of actions a user can take, such as the overflow actions of a row or a card. When the user is picking a value rather than performing an action, reach for [the `Select` component](./select.mdx), or [the `Combobox` component](./combobox.mdx) if the list is long enough to warrant searching it. When one action is common enough that it deserves a button of its own, reach for [the `SplitButton` component](./split-button.mdx), which pairs a primary action with a menu of related ones.
...
Clicking the trigger opens the menu, and choosing an item fires its `onSelect` and closes the menu again. A `Menu` is also fully keyboard operable: `Enter` or `Space` opens it from the trigger, the arrow keys move between items, typing jumps to the item whose `text` matches, and `Escape` closes the menu and returns focus to the trigger.

Any element that can be focused and receive a ref works as a trigger, which makes an [Icon Button](./icon-button.mdx) a natural fit for overflow menus:
...
export default function IconTriggerMenu() {
  return (
    <Menu
      items={[
        { text: "Share", onSelect: () => {} },
        { text: "Move to folder", onSelect: () => {} },
        { text: "Archive", onSelect: () => {} },
      ]}
    >
      <IconButton
...
Each item can render an `icon` to the left of its label, and a `description` underneath it for the items whose outcome is not obvious from their label alone:
...
Descriptions are optional, and can be mixed with items that only have a label. Keep them to a single short sentence: the menu does not grow to fit its content, so anything longer wraps onto several lines.
...
> [!NOTE]
>
> `"locked"` and `"disabled"` items cannot be activated: their `onSelect` never fires, and the menu stays open when one of them is clicked. They remain reachable with the keyboard, so assistive technology can still announce them and their state. `onSelect` is required for them all the same, since the same item usually becomes selectable again as the user's context changes.

### Setting the width

A menu is `200px` wide by default. Pass `width` to change that, which is useful when items carry descriptions or unusually long labels. This value scales with the base rem size:
...
export default function MenuWidths() {
  return (
    <div style={{ display: "flex", gap: "var(--vesper-spacing-4)" }}>
      <Menu items={ITEMS}>
        <Button variant="subtle">Default width</Button>
      </Menu>
      <Menu width={320} items={ITEMS}>
        <Button variant="subtle">320px wide</Button>
      </Menu>
    </div>
  );
}
      ...
      >
        <Button
          ...
          style={{
            ...
            marginBottom: "var(--vesper-spacing-16)",
          }}
        >
          Open menu
        </Button>
...
If you need to own the open state, use the `open` and `onOpenChange` props together. `onOpenChange` fires with the state the menu wants to move to, which keeps the usual interactions (clicking the trigger, selecting an item, pressing `Escape`) working while you stay in control:
...
A menu is positioned against its trigger by default. Pass `anchor` an element (or a ref to one) to position it against something else, which is handy when the trigger is part of a larger control that the menu should line up with:
...
The menu above is aligned to the end of the card rather than the end of the small icon button that opens it. This is exactly how [the `SplitButton` component](./split-button.mdx) positions its own menu.

### Rendering the menu in another container

A menu renders in a portal, attached to `document.body`, so it is never clipped by the overflow of the elements it sits in. Pass `container` an element, a shadow root, or a ref to either when it needs to be portalled somewhere else, eg. into a shadow root that holds your app's styles:
