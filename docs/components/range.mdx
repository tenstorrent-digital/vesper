...
Additional props are forwarded as follows:

- `aria-labelledby` is forwarded to the first range thumb. This ensures that clicking the element corresponding to `aria-labelledby` brings focus inside the range to the first thumb.
- `aria-describedby` is forwarded to **all** range thumbs. This ensures that an element describing the range is announced no matter which thumb is currently being interacted with.
