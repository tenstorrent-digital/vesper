...
The most straightforward way to render a `Tooltip` is to simply render some children inside of it and give it some `content`. `content` can be any `ReactNode`, though it is highly advisable to just render plain text:
...
You can force a `Tooltip`'s content to show by default prior to user interaction by passing `defaultOpen={true}` or just `defaultOpen`. The `Tooltip` will stay visible until the user hovers over and leaves the trigger, or if the user clicks somewhere on the page:
