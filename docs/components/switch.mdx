...
<Switch size="sm" text="A small switch" />
<Switch size="md" text="A medium switch" />
```
...
`Switch` renders a native `<input type="checkbox">`, so it works with regular form submission and native validation via props like `name`, `value`, `form`, and `required`. A switch that has been turned on its `value` (defaulting to `"on"`), and a turned off switch is omitted from form data entirely.
