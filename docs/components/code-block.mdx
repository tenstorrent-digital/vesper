...
export default function BuildLogs({ buildLogs }: { buildLogs: string }) {
  return <CodeBlock lang="ansi">{buildLogs}</CodeBlock>;
}
```
...
export default function StreamedCodeBlock({
  ...
  getCodeStream: () => ReadableStream<string>;
}) {
  return <CodeBlock lang={typescript}>{() => getCodeStream()}</CodeBlock>;
}
```
