...
This is the same component used to render the `message` of [Text Input](./text-input.mdx) and [Textarea](./textarea.mdx), so you'll usually only reach for it directly when describing a form control that Vesper doesn't provide.
...
Note that [Text Input](./text-input.mdx) and [Textarea](./textarea.mdx) already render a `FormInputMessage` for you and wire up `aria-describedby` automatically, so prefer passing `message` and `variant` to those components directly:
